...
class MainWindow(Screen):
    ...
    def NFC_detection(status):                           ##<< CHANGING SCREEN - use it when detection
        sm.current="return" if status == "NOT_AVAILABLE" else "renting"
        sm.transition.direction="up" if status == "NOT_AVAILABLE" else "left"
...
class ReturnWindow(Screen):
    ...
    def on_enter(self):
        ...
        self.user_data = user      ##<<<<HERE I NEED USER DATA
        self.dev_data = part       ##<<<<HERE I NEED DEVICE DATA
    ...
    #    self.us_project.text = self.user_data["project"] #TODO: We're not using project!
